#!/usr/bin/env python3
"""Build V6 portfolio deliverables.

Delta from V5:
- Cover author block (劉晏禎 / miles891002@gmail.com).
- KPI strip adds FRR best diagnostic (6 chips).
- 4 new challenging QnA: look-ahead/data snooping, survivorship/universe,
  capacity/market impact, sector regime shift.
- Footer/subtitle/version bump to V6.
- ROOT path points at the v6 copy.
"""
...
ROOT=Path('/Users/liuyenzhen/quant-research/tw_monthly_revenue_v6')
PORT=ROOT/'portfolio_project'; CHARTS=ROOT/'reports'/'charts'; V3CHARTS=PORT/'charts_v3'; PROCESSED=ROOT/'data'/'processed'
RESUME_DOCX=PORT/'Taiwan_Monthly_Revenue_Strategy_Resume_Version_ZH_PRO_V6.docx'
RESUME_PDF=PORT/'Taiwan_Monthly_Revenue_Strategy_Resume_Version_ZH_PRO_V6.pdf'
GUIDE_DOCX=PORT/'Taiwan_Monthly_Revenue_Strategy_Talking_Guide_ZH_PRO_V6.docx'
GUIDE_PDF=PORT/'Taiwan_Monthly_Revenue_Strategy_Talking_Guide_ZH_PRO_V6.pdf'

AUTHOR_NAME='劉晏禎 (Liu Yen-Zhen)'
AUTHOR_EMAIL='miles891002@gmail.com'
VERSION_TAG='V6'
...
def tables():
    ...
    'qa':[
      ...
      ['Look-ahead / data snooping 怎麼防？','所有 feature 都用月營收公布日之後可觀察的時點：SUR 用 t-1 月之前的 12 期、進場最早 next-open、Phase 3.19 還做 delay 1–3 與 1.0% 成本壓力。Phase 3 多輪搜尋我承認有 data snooping 風險，因此 (a) 保留 S1 簡潔 incumbent 不被新 variant 覆寫；(b) walk-forward train-selected rule 沒有穩定打敗固定 S1，正是這個風險被量化的結果。'],
      ['Universe 是不是 survivorship bias？','目前 universe 是 2020-2025 仍上市的母體，下市/減資/合併樣本沒補齊，所以我在 06 章直接把 survivorship 標為「中-高」風險，不宣稱 broad-market alpha。下一步要從 TWSE/TPEx 取歷史 delisted list 重建 point-in-time universe，再跑一輪 OOS。'],
      ['Capacity 與市場衝擊怎麼估？','S1 是 Top 8、20D 持有、流動性 gate 已排除低成交金額標的。粗估單檔每天可吃 5–10% 日均成交金額，AUM 上限約落在 1–3 億台幣，再上去 market impact 與 next-open 漲停 non-fill 會把實測 Sharpe 拉到回測下方。所以這個策略定位是 capacity-limited research candidate，不是可堆規模的 production alpha。'],
      ['Sector regime 轉換時還有意義嗎？','S1 在電子/半導體佔比明顯偏高，AI 多頭結束或半導體進入 down-cycle 時績效會顯著下滑——這在 sector survival 圖已可見。我的處理是 (a) 對外不稱 broad alpha；(b) 把 industry cap 寫進選股流程；(c) 把 sector breakdown 放進月度 committee 流程；(d) 未來研究方向就是非電子訊號的補強與 regime filter，但這在 paper trading 之前不會宣稱已解決。']]
    }
...
def doc_content(doc,title,sub,is_guide,ch,tb):
    ...
    auth=doc.add_paragraph(f'{AUTHOR_NAME}｜{AUTHOR_EMAIL}｜{VERSION_DATE}'); auth.alignment=WD_ALIGN_PARAGRAPH.CENTER
    for r in auth.runs: r.font.bold=True
    doc.add_paragraph('0050/高股息ETF 作品集版式參照｜Research / Paper-Trading Only｜非投資建議').alignment=WD_ALIGN_PARAGRAPH.CENTER
    doc.add_paragraph('25 個月樣本｜S1 Sharpe≈2.40｜FRR best +251.4% (diagnostic)｜12 張圖表｜完整 promotion gates').alignment=WD_ALIGN_PARAGRAPH.CENTER; doc.add_page_break()
    ...
    if is_guide:
        ...
        doc.add_heading('挑戰性 QnA（含 4 題硬核質疑）',2)
        dtable(doc,['問題','回答'],tb['qa'])

# PDF
def make_styles():
    s=getSampleStyleSheet();
    s.add(ParagraphStyle('TitleC',fontName=FONT,fontSize=23,leading=30,textColor=NAVY,alignment=TA_CENTER,spaceAfter=8))
    s.add(ParagraphStyle('SubC',fontName=FONT,fontSize=10.5,leading=15,textColor=MUTED,alignment=TA_CENTER,spaceAfter=6))
    s.add(ParagraphStyle('AuthC',fontName=FONT,fontSize=11,leading=15,textColor=NAVY,alignment=TA_CENTER,spaceAfter=4))
    s.add(ParagraphStyle('H1C',fontName=FONT,fontSize=14.2,leading=19,textColor=NAVY,spaceBefore=7,spaceAfter=5))
    s.add(ParagraphStyle('BodyC',fontName=FONT,fontSize=8.7,leading=12.7,textColor=colors.HexColor('#111827'),wordWrap='CJK',spaceAfter=4))
    s.add(ParagraphStyle('SmallC',fontName=FONT,fontSize=7.1,leading=9.4,textColor=MUTED,wordWrap='CJK'))
    s.add(ParagraphStyle('CapC',fontName=FONT,fontSize=7.1,leading=9.3,textColor=MUTED,alignment=TA_CENTER,spaceAfter=5))
    return s
S=make_styles()
...
def ptable(headers,rows,widths=None):
    ...
    t.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),NAVY),('TEXTCOLOR',(0,0),(-1,0),WHITE),('ROWBACKGROUNDS',(0,1),(-1,-1),[WHITE,LIGHT]),('BOX',(0,0),(-1,-1),.45,BORDER),('INNERGRID',(0,0),(-1,-1),.25,BORDER),('VALIGN',(0,0),(-1,-1),'TOP'),('LEFTPADDING',(0,0),(-1,-1),3.5),('RIGHTPADDING',(0,0),(-1,-1),3.5),('TOPPADDING',(0,0),(-1,-1),4),('BOTTOMPADDING',(0,0),(-1,-1),4)]))
    return t
...
def footer(canvas,doc):
    ...
    canvas.drawString(1.55*cm,.72*cm,f'Taiwan Monthly Revenue Surprise Research｜{VERSION_TAG} Professional Portfolio｜{AUTHOR_NAME}｜Research Only')
    canvas.drawRightString(A4[0]-1.55*cm,.72*cm,f'Page {doc.page}'); canvas.restoreState()
def build_pdf(path,title,sub,is_guide,ch,tb):
    story=[Spacer(1,1.2*cm),
        Paragraph(esc(title),S['TitleC']),
        Paragraph(esc(sub),S['SubC']),
        Paragraph(esc(f'{AUTHOR_NAME}　｜　{AUTHOR_EMAIL}　｜　{VERSION_DATE}'),S['AuthC']),
        Spacer(1,.15*cm),
        ptable(['S1 Sharpe','Total Return','MDD','FRR best','圖表','狀態'],
               [['≈2.40','+167.5%','-7.9%','+251.4% (diag.)','12','Research candidate']], [2.7*cm]*6),
        Spacer(1,.25*cm),
        P('Research / Paper-Trading Only｜非投資建議｜非實盤交易系統','SmallC'),
        PageBreak()]
    ...
    if is_guide:
        story+=[PageBreak()]+sect('八、面試講稿與答辯')
        story+=[P('30 秒版本：我做的是台股月營收 surprise 的短週期量化研究。核心從台灣每月公布營收的制度出發，測試持續性營收驚喜是否會在 10–20 個交易日內被市場逐步重估。我不只看 Sharpe，也做 OOS、sector survival、remove-winner、execution realism、融資策略延伸與 paper-trading schema，所以最後把 S1 定位為 portfolio-grade research candidate，而非 production-ready alpha。')]
        story+=[Paragraph('挑戰性 QnA（含 4 題硬核質疑）',S['H1C'])]
        story+=[ptable(['問題','回答'],tb['qa'],[3.8*cm,13.1*cm])]
...
def main():
    ...
    sub_res=f'履歷附件版 {VERSION_TAG}｜參照 ETF 作品集版式與敘事密度｜{VERSION_DATE}'
    sub_guide=f'面試講稿版 {VERSION_TAG}｜參照 ETF 作品集答辯風格｜{VERSION_DATE}'
    d=Document(); doc_content(d,'台股月營收驚喜策略研究',sub_res,False,ch,tb); d.save(RESUME_DOCX)
    g=Document(); doc_content(g,'台股月營收驚喜策略研究',sub_guide,True,ch,tb); g.save(GUIDE_DOCX)
    build_pdf(RESUME_PDF,'台股月營收驚喜策略研究',sub_res,False,ch,tb)
    build_pdf(GUIDE_PDF,'台股月營收驚喜策略研究',sub_guide,True,ch,tb)
